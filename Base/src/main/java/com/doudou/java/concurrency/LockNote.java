 ...
 *
 * 每个内置锁只能对应一个内置条件队列，因此可能存在多个线程在同一个条件队列上
 * 等待不同的条件谓词（等待不同的条件变为真），在并发编程的有界队列缓存中使用了
 * wait来等待，两个条件谓词，一个是队满不能继续put，一个是队空不能继续take，所以
 * 当一个条件为真时notifyAll会通知两种类型的线程都来竞争锁，这是不必要的。
 *
 * 使用两个condition就可以在两个条件队列上等待不同的条件变为真。
 *
 *
 * @author doudou
 */
